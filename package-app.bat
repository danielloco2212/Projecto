...
if %ERRORLEVEL% neq 0 (
    echo.
    echo [ERROR] La compilacion con Maven ha fallado. Revisa los errores arriba.
    pause
    exit /b %ERRORLEVEL%
)
...
if %ERRORLEVEL% neq 0 (
    echo [ERROR] No se encuentra 'candle.exe'. Asegurate de que WiX Toolset v3.x este instalado y su carpeta 'bin' añadida al PATH.
    pause
    exit /b 1
)
...
if exist target\installer-input rd /s /q target\installer-input
...
for %%f in (target\*.jar) do (
    if not "%%~nf"=="original-%%~nf" (
        copy "%%f" "target\installer-input\" > nul
        set JAR_NAME=%%~nxf
    )
)

if "%JAR_NAME%"=="" (
    echo [ERROR] No se pudo encontrar el archivo JAR en target.
    pause
    exit /b 1
)
